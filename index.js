...
async function start() {
	try {
		...
		//=========================================================================
		//вторая страница
		await page.click('#table_event_results_final_paginate > ul > li:nth-child(3) > a');
		await page.waitForTimeout(6000);
		await page.screenshot({ path: './image.jpg' })


		const table2 = await page.evaluate(async (linkZp) => {
			//определение названий столбцов и формирование массива
			resultThead = [];
			let thead = await document.querySelectorAll('#table_event_results_final > thead > tr > th');
			thead.forEach(element => {
		...
		fs.appendFile('zwiftpower.json', JSON.stringify(table2), (error) => {
			if (error) {
				throw error
			}
		})
